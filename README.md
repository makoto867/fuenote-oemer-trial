...
無料プランの512MB内で試験できるよう、各ONNXモデルを個別プロセスで完了させてからoemer本体を読み込み、推論パッチを逐次統合する低メモリ実行を使用します。中間マスクも8ビットに圧縮します。認識アルゴリズムとMusicXML生成はoemerのままです。

調整用の環境変数は `OEMER_TARGET_PIXELS`（既定100万画素）、`OEMER_BATCH_SIZE`（既定1）、`OEMER_STEP_SIZE`（既定192）です。
